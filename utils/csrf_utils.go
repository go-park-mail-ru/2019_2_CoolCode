...
func init() {
	Tokens, _ = NewHMACHashToken("CoolSlackToken")
}
...
func (tk *HashToken) Create(userID uint64, cookie string, tokenExpTime int64) (string, error) {
	h := hmac.New(sha256.New, []byte(tk.Secret))
	data := fmt.Sprintf("%d:%s:%d", userID, cookie, tokenExpTime)
	h.Write([]byte(data))
	token := hex.EncodeToString(h.Sum(nil)) + ":" + strconv.FormatInt(tokenExpTime, 10)
	return token, nil
}

func (tk *HashToken) Check(userID uint64, cookie string, inputToken string) (bool, error) {
	...
	h := hmac.New(sha256.New, []byte(tk.Secret))
	data := fmt.Sprintf("%d:%s:%d", userID, cookie, tokenExp)
	h.Write([]byte(data))
